export async function fetchMetrixData(metrixIdOrUrl: string ) {
  ...
  const parseMetrixId = (): number | null => {
      if (metrixIdOrUrl.toLowerCase().includes('discgolfmetrix.com') || metrixIdOrUrl.toLowerCase().includes('dgmtrx.com')) {
  ...
  const response = await fetch(`https://discgolfmetrix.com/api.php?content=result&id=${parseMetrixId()}`);
